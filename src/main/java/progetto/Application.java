...
public class Application {
	...
	public static void main(String[] args) {
		...
		/*
		 * cd.save(libro1); cd.save(libro2); cd.save(rivista1); cd.save(rivista2);
		 * 
		 * ud.save(utente1); ud.save(utente2); ud.save(utente3); ud.save(utente4);
		 * 
		 * pd.save(prestito1); pd.save(prestito2); pd.save(prestito3);
		 * pd.save(prestito4); pd.save(prestito5);
		 * 
		 * cd.delete(UUID.fromString("216d34a7-7a44-4c01-b205-66f9de3a7688"));
		 * 
		 * log.info(
		 * "***************************************TROVATO PER ANNO************************************************"
		 * ); List<Catalogo> trovatoPerAnno = cd.getByYear(2022); if
		 * (trovatoPerAnno.size() > 0) { trovatoPerAnno.stream().forEach(c ->
		 * log.info(c.toString())); } else {
		 * log.info("NESSUN CATALOGO TROVATO SCRITTO NELL' ANNO INDICATO"); }
		 * 
		 * log.info(
		 * "***************************************TROVATO PER TITOLO************************************************"
		 * ); List<Catalogo> trovatoPerTitolo = cd.getByTitle("Harry"); if
		 * (trovatoPerTitolo.size() > 0) { trovatoPerTitolo.stream().forEach(c ->
		 * log.info(c.toString())); } else {
		 * log.info("NESSUN CATALOGO TROVATO AVENTE IL TITOLO INDICATO"); }
		 * 
		 * log.info(
		 * "**************************************TROVATO PER AUTORE***************************************************"
		 * ); List<Libro> trovatoPerAutore = cd.getByAuthor("J.K.Rowling"); if
		 * (trovatoPerAutore.size() > 0) { trovatoPerAutore.stream().forEach(l ->
		 * log.info(l.toString())); } else {
		 ...
		 * else { log.
		 * info("NESSUN PRESTITO TROVATO PER L' UTENTE AVENTE IL NUMERO DI TESSERA INSERITO"
		 * ); }
		 * 
		 * log.info(
		 * "**************************************PRESTITO SCADUTO TROVATO***************************************************"
		 * ); List<Prestito> trovatoPrestitiScaduti =
		 * pd.getPrestitiScadutiENonRestituiti();
		 * 
		 * if (trovatoPrestitiScaduti.size() > 0) {
		 * trovatoPrestitiScaduti.stream().forEach(c -> log.info(c.toString())); } else
